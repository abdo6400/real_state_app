...
class TypeTimeSection extends StatelessWidget {
  const TypeTimeSection({super.key});
  ...
  Widget build(BuildContext context) {
    return Wrap(
      ...
      children: List.generate(
        context.read<EstateCubit>().titlesTime.length,
        (index) => TimeCard(
          title: context.read<EstateCubit>().titlesTime[index],
          index: index,
        ),
      ),
    );
...
class TimeCard extends StatelessWidget {
  final String title;
  final int index;
  const TimeCard({super.key, required this.title, required this.index});

  @override
  Widget build(BuildContext context) {
    final isSelected = index == context.watch<EstateCubit>().timeIndex;
    return GestureDetector(
      key: ValueKey(title.codeUnits),
      onTap: () => context.read<EstateCubit>().changeTimeIndex(index),
      child: Card(
        elevation: 0,
